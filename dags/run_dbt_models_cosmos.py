...
# Command to install dbt dependencies
install_dbt_deps_command = f"cd {dbt_project} && dbt deps"

# Command to clean dbt project
clean_dbt_command = f"cd {dbt_project} && dbt clean"
...
with DAG(
    ...
    # Tâche pour installer les dépendances dbt
    install_dbt_deps = BashOperator(
        task_id='install_dbt_deps',
        bash_command=install_dbt_deps_command
    )
    ...
    start >> install_dbt_deps  >> dbt_tg >> run_dbt_tests_models >> copy_dbt_target_files_to_s3() >> end